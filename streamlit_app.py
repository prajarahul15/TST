...
# Helper functions for stock price retrieval
def get_stock_price_alpha_vantage(symbol: str, api_key: str) -> List[str]:
    """Get stock price from Alpha Vantage with robust error handling"""
    try:
        ...
        if response.status_code == 200:
            ...
            if 'Error Message' in data:
                return [f"⚠️ Alpha Vantage Error: {data['Error Message']}"]
            
            if 'Note' in data:
                return [f"⚠️ API Rate Limit: {data['Note']}", "Please try again in a few minutes."]
            
            quote = data.get('Global Quote', {})
            if quote and '05. price' in quote:
                return [
                    f"📈 **{symbol} Stock Quote (Alpha Vantage)**",
                    f"   Current Price: ${float(quote.get('05. price', 0)):.2f}",
                    f"   Change: {quote.get('09. change', 'N/A')}",
                    f"   Change %: {quote.get('10. change percent', 'N/A')}",
                    f"   Volume: {quote.get('06. volume', 'N/A')}",
                    f"   Last Updated: {quote.get('07. latest trading day', 'N/A')}",
                    f"   Source: Alpha Vantage",
                    ""
                ]
            else:
                return [f"⚠️ No stock data found for {symbol} in Alpha Vantage response"]
                
        elif response.status_code == 202:
            return [
                f"⏳ **{symbol} Request Processing (HTTP 202)**",
                "The Alpha Vantage API has accepted your request but is still processing it.",
                "This usually happens during high-traffic periods.",
                "**Recommendation**: Try again in 30-60 seconds.",
                "**Alternative**: Use Yahoo Finance or Google Finance for immediate data.",
                ""
            ]
        elif response.status_code == 429:
            return [
                f"🚫 **Rate Limit Exceeded for {symbol}**",
                "You've reached the API rate limit (5 calls per minute for free tier).",
                "**Solutions**:",
                "• Wait 1 minute before trying again",
                "• Upgrade to Alpha Vantage premium for higher limits",
                "• Use alternative data sources below",
                ""
            ]
        else:
        ...
        return [f"⏰ Timeout error when fetching {symbol} from Alpha Vantage"]
    except requests.exceptions.RequestException as e:
        return [f"🌐 Network error when fetching {symbol}: {str(e)}"]
    except Exception as e:
        return [f"❌ Unexpected error fetching {symbol}: {str(e)}"]

def get_stock_price_yahoo_fallback(symbol: str) -> List[str]:
    """Fallback method using Yahoo Finance (no API key required)"""
    try:
        # Yahoo Finance doesn't require API key but has limitations
        ...
        if response.status_code == 200:
            ...
            if 'chart' in data and data['chart']['result']:
                ...
                if 'regularMarketPrice' in meta:
                    ...
                    return [
                        f"📊 **{symbol} Stock Quote (Yahoo Finance Fallback)**",
                        f"   Current Price: ${current_price:.2f}",
                        f"   Change: ${change:+.2f}",
                        f"   Change %: {change_percent:+.2f}%",
                        f"   Previous Close: ${previous_close:.2f}",
                        f"   Currency: {meta.get('currency', 'USD')}",
                        f"   Source: Yahoo Finance (Fallback)",
                        f"   ⚠️ Note: For more detailed data, configure Alpha Vantage API key",
                        ""
                    ]
        
        return [f"❌ Could not retrieve {symbol} data from Yahoo Finance fallback"]
        
    except Exception as e:
        return [f"❌ Yahoo Finance fallback error for {symbol}: {str(e)}"]

# Economic Data and Web Search Tools
@tool
def search_economic_indicators(query: str, max_results: int = 5) -> str:
    """
    Search for economic indicators and macro/microeconomic data using FRED API and other sources.
    ...
    try:
        ...
        # If we have FRED API key, use it for economic indicators
        if fred_api_key and any(indicator in query.lower() for indicator in ['inflation', 'gdp', 'unemployment', 'interest rate', 'cpi']):
            try:
                from fredapi import Fred
                fred = Fred(api_key=fred_api_key)
                
                # Map common queries to FRED series IDs
                fred_indicators = {
                ...
                for keyword, series_id in fred_indicators.items():
                    if keyword in query.lower():
                        try:
                            data = fred.get_series(series_id, limit=6)
                            if not data.empty:
                                ...
                                results.append(f"   Latest Value: {latest_value:.2f}")
                                results.append(f"   Trend: {'📈' if trend == 'up' else '📉'} {trend.title()}")
                                results.append(f"   Source: Federal Reserve Economic Data (FRED)")
                                results.append("")
                        except Exception as e:
                            print(f"Error fetching {keyword}: {e}")
                            
            except ImportError:
                print("fredapi not installed, using fallback method")
        
        # Enhanced stock price retrieval with better error handling
        stock_symbols = ['AAPL','FIS', 'MSFT', 'GOOGL', 'TSLA', 'AMZN', 'NVDA', 'META', 'NFLX', 'AMD', 'INTC']
        if any(symbol in query.upper() for symbol in stock_symbols):
            ...
            if target_symbol:
                ...
                if alpha_vantage_key:
                    stock_data = get_stock_price_alpha_vantage(target_symbol, alpha_vantage_key)
                    if stock_data:
                        results.extend(stock_data)
                
                # If Alpha Vantage failed or no API key, try Yahoo Finance fallback
                if not results:
                    stock_data = get_stock_price_yahoo_fallback(target_symbol)
                    if stock_data:
                        results.extend(stock_data)
        
        # If we have results from APIs, return them
        if results:
            return "\n".join(results)
        
        # Enhanced fallback response with specific guidance
        is_stock_query = any(symbol in query.upper() for symbol in stock_symbols)
        
        if is_stock_query:
            return f"""
📊 **Stock Price Search Results for: {query}**

⚠️  **Unable to retrieve stock data at this time**

**Possible Reasons:**
• Alpha Vantage API key not configured
• API rate limit exceeded (5 calls per minute for free tier)
• Temporary API service issues (HTTP 202 processing delay)

**🚀 Immediate Solutions:**

**1. Manual Stock Price Check:**
• Yahoo Finance: https://finance.yahoo.com/quote/{target_symbol if 'target_symbol' in locals() else 'SYMBOL'}
• Google Finance: https://www.google.com/finance/quote/{target_symbol if 'target_symbol' in locals() else 'SYMBOL'}:NASDAQ
• Bloomberg: https://www.bloomberg.com/quote/{target_symbol if 'target_symbol' in locals() else 'SYMBOL'}:US

**2. Configure Alpha Vantage API (Free):**
• Get API key: https://www.alphavantage.co/support/#api-key
• Add to .env file: ALPHA_VANTAGE_API_KEY=your_key_here
• Restart the application

**3. If you have API key but getting HTTP 202:**
• Wait 30-60 seconds and try again
• Alpha Vantage may be processing your request during high traffic
...
"""
            ...
            return f"""
📊 **Economic Indicators Search Results for: {query}**

⚠️  **API Setup Required**: For real-time data, please configure API keys in your .env file:

**Required API Keys:**
• FRED_API_KEY (for economic indicators like inflation, GDP, unemployment)
• ALPHA_VANTAGE_API_KEY (for stock data and market information)
• NEWS_API_KEY (for market news and sentiment)

📈 **Available Data Sources**:
1. **FRED API**: Official US economic indicators
2. **Alpha Vantage**: Stock prices and company data  
3. **News API**: Market sentiment and financial news
...
3. Install packages: pip install fredapi alpha-vantage newsapi-python

💡 **Demo Response for "{query}"**:
Based on typical economic indicators, this query would return current market trends, policy impacts, and regional economic variations. With proper API integration, you would receive real-time data including specific values, trends, and analysis.
"""
...
def get_industry_analysis(industry: str, region: str = "US") -> str:
    """
    Get comprehensive industry analysis and trends using multiple data sources.
    ...
    try:
        ...
        industry_codes = {
            'cloud computing': 'NAICS 518210',
            'automotive': 'NAICS 3361', 
            'healthcare': 'NAICS 62',
            'financial services': 'NAICS 52',
            'retail': 'NAICS 44-45',
            'manufacturing': 'NAICS 31-33',
            'technology': 'NAICS 54151',
            'energy': 'NAICS 211',
            'telecommunications': 'NAICS 517'
        }
        
        naics_code = industry_codes.get(industry.lower(), 'Not classified')
        
        # If we have Alpha Vantage key, try to get sector performance
        if alpha_vantage_key:
            try:
                ...
                if response.status_code == 200:
                    ...
                    for key, sector_name in sector_mapping.items():
                        if key in industry.lower():
                            rank_data = data.get('Rank A: Real-Time Performance', {})
                            if sector_name in rank_data:
                                results.append(f"📈 **{sector_name} Sector Performance**")
                                results.append(f"   Real-time Performance: {rank_data[sector_name]}")
                                results.append(f"   Source: Alpha Vantage Sector Performance")
                                results.append("")
                                has_real_data = True
                            break
        ...
        # If we have News API key, get industry news
        if news_api_key:
            try:
                url = 'https://newsapi.org/v2/everything'
                params = {
                    'q': f"{industry} industry trends",
                    'sources': 'bloomberg,reuters,financial-times,techcrunch',
                    'sortBy': 'publishedAt',
                    'pageSize': 3,
                    'apiKey': news_api_key,
                    'language': 'en'
                ...
                if response.status_code == 200:
                    ...
                    if articles:
                        results.append("📰 **Recent Industry News**:")
                        for article in articles[:2]:
                            results.append(f"• {article.get('title', 'No title')}")
                            results.append(f"  Source: {article.get('source', {}).get('name', 'Unknown')}")
                            results.append(f"  Date: {article.get('publishedAt', 'Unknown')[:10]}")
                        results.append("")
                        has_real_data = True
...
**Industry Classification**: {naics_code}
...
"""
            ...
            # Even without API results, provide a comprehensive framework analysis
            analysis += f"""

**Professional Industry Analysis Framework**:

📊 **Market Overview for {industry.title()}**:
• Industry Classification: {naics_code}
• Geographic Focus: {region}
...
• Innovation metrics and R&D spending
• Market share dynamics among key players

💡 **Strategic Analysis Framework**:
• Porter's Five Forces competitive analysis
• SWOT analysis for sector positioning
...
• Global economic and trade considerations"""

            analysis += f"""

**Data Sources and Recommendations**:
• Industry trade associations and reports
• Government economic and sector data
• Company financial filings and earnings calls
• Market research and consulting reports
• Academic research and policy papers

**For Enhanced Real-Time Analysis**:
Configure API keys in .env file for live market data, news sentiment, and sector performance metrics."""
...
def get_market_sentiment(topic: str) -> str:
    """
    Get market sentiment and news analysis for specific topics using News API.
    ...
    try:
        ...
        if news_api_key:
            try:
                # Get recent economic news from News API
                url = 'https://newsapi.org/v2/everything'
                params = {
                    'q': topic,
                    'sources': 'bloomberg,reuters,financial-times,wall-street-journal,cnbc',
                    'sortBy': 'publishedAt',
                    'pageSize': 5,
                    'apiKey': news_api_key,
                    'language': 'en'
                ...
                if response.status_code == 200:
                    ...
                    if articles:
                        ...
                        positive_words = ['growth', 'increase', 'positive', 'bullish', 'gains', 'up', 'strong', 'boost']
                        negative_words = ['decline', 'decrease', 'negative', 'bearish', 'losses', 'down', 'weak', 'fall']
                        ...
                        for article in articles[:3]:  # Analyze top 3 articles
                            ...
                            news_summary.append(f"• {article.get('title', 'No title')} - {sentiment_label}")
                            news_summary.append(f"  Source: {article.get('source', {}).get('name', 'Unknown')}")
                            news_summary.append(f"  Published: {article.get('publishedAt', 'Unknown')[:10]}")
                            news_summary.append("")
                        
                        avg_sentiment = sum(sentiment_scores) / len(sentiment_scores) if sentiment_scores else 0
                        
                        overall_sentiment = "Positive 📈" if avg_sentiment > 0.1 else "Negative 📉" if avg_sentiment < -0.1 else "Neutral ➡️"
                        
                        return f"""
📰 **Market Sentiment Analysis: {topic.title()}**

**Overall Sentiment**: {overall_sentiment}
**Sentiment Score**: {avg_sentiment:.2f} (Range: -1 to +1)
**Analysis Date**: {datetime.now().strftime('%Y-%m-%d %H:%M')}
...
**Sentiment Breakdown**:
• Positive signals: {sentiment_scores.count(1)} articles
• Negative signals: {sentiment_scores.count(-1)} articles  
• Neutral signals: {sentiment_scores.count(0)} articles

**Data Source**: News API (Bloomberg, Reuters, Financial Times, WSJ, CNBC)
"""
                        
                    else:
                        return f"No recent news articles found for '{topic}'. Try different keywords or check API limits."
                        
                else:
                    return f"Error fetching news data: HTTP {response.status_code}"
...
⚠️  **API Setup Required**: For real-time sentiment analysis, please configure NEWS_API_KEY in your .env file.

**Setup Instructions**:
1. Get free API key from: https://newsapi.org/register
2. Add NEWS_API_KEY=your_key_here to your .env file
3. Install package: pip install newsapi-python

**Available Features with API**:
• Real-time news sentiment analysis
• Multiple financial news sources (Bloomberg, Reuters, FT, WSJ)
• Sentiment scoring (-1 to +1 scale)
• Article-level sentiment breakdown
• Publication date tracking

**Demo Analysis for "{topic}"**:
Based on typical market sentiment patterns, this topic would show current market mood, news impact, and investor sentiment. With proper API integration, you would receive:
- Real news headlines and sentiment scores
- Source attribution from major financial publications
...
class FinancialInsightOrchestrator:
    ...
    def _create_agent(self):
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a Financial Data Analysis Expert with access to both internal financial data and external economic/market data. Your role is to:
            
            **For Internal Data Questions:**
            ...
            **For External Data Questions (stocks, market data, economic indicators):**
            5. Use search_economic_indicators tool for stock prices, market data, and economic indicators
            6. Use get_industry_analysis tool for industry-specific trends and analysis
            7. Use get_market_sentiment tool for current market sentiment and news
            8. ALWAYS attempt to use these tools first for external market queries before saying data is unavailable
            
            **Available External Data Tools:**
            - search_economic_indicators: Search for stock prices, economic data, market indicators
            - get_industry_analysis: Get industry-specific analysis and trends
            - get_market_sentiment: Get current market sentiment and financial news
            
            When users ask about stock prices, market data, or economic indicators, ALWAYS use the appropriate external data tools to provide real information instead of generic responses.
        ...
        return create_tool_calling_agent(llm=self.llm, tools=self.tools, prompt=prompt)
    ...
    def ask_question(self, user_question: str, context_files: list = None) -> str:
        ...
        try:
            if context_files:
                ...
                **Instructions:**
                1. For internal financial data questions: Use load_and_prepare_data, get_data_statistics, analyze_trends, create_visualization, perform_variance_analysis
                2. For external market/stock questions: Use search_economic_indicators, get_industry_analysis, get_market_sentiment tools
                3. ALWAYS try external data tools first for stock prices, market data, economic indicators
                4. Analyze the data to answer the user's question comprehensively
                5. Provide clear, business-focused insights with real data when available
                6. If specific data is unavailable after using tools, explain what you searched for and suggest alternatives
    ...
    with tab6:
        ...
        try:
            ...
            example_questions = [
                ...
                "Get me the latest market sentiment for tech stocks",
                "What are the current economic indicators for inflation?",
                "Analyze the technology industry trends"
            ]
